...
(async () => {
  ...
  try {
    ...
    const leaderPanel = page.getByRole("region", { name: "Vim leader" });
    assert.ok(await leaderPanel.getByText("Space", { exact: true }).isVisible());
    assert.equal(await page.getByTitle("Vim leader key").count(), 1);
    await assertWindowLayout(page);
    await page.screenshot({ path: path.join(artifacts, "vim-leader.png") });
    await page.keyboard.press("Space");
    assert.ok(await leaderPanel.getByText("Leader received", { exact: false }).isVisible());
    assert.equal(await page.getByTitle("Vim leader key").count(), 0);
    await page.screenshot({ path: path.join(artifacts, "vim-leader-ready.png") });
    ...
    assert.equal(saved.profiles.find((profile) => profile.kind === "vim").vimLeader, "<Space>");
